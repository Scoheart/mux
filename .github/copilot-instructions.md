...
- Keep npm lockfiles committed and use `npm ci`. Stable publication keeps immutable-tag provenance, version, signing, App/DMG, updater, CLI, asset-set, and latest-channel checks; Quality runs asynchronously from the same stable tag. Never replace full-SHA Action pins with mutable tags.
