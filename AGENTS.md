...
MUX 统一管理编码 Agent 的 MCP servers 和模型端点。Rust core 是行为与数据的唯一权威，CLI/TUI 和 Tauri command 只做薄适配。

| 路径 | 归属 |
|---|---|
| `core/` | Agent 发现、配置 codec、MCP/model 写入、备份与安全不变量 |
| `cli/` | CLI/TUI 展示与命令路由 |
...
- 当前只管理 Agent 的全局配置。项目级 metadata 仅作兼容读取，不新增项目级写入入口。
...
- 顶层产品区为 `MCPs` 与 `Models`；两者复用一致的 resource workspace、筛选、卡片层级和编辑模式。
- Agent 页面是配置中心：同页展示配置路径、MCP 路径、模型分配与 MCP 管理，避免要求用户跨页面来回操作。
...
- 不提交 `target/`、`dist/`、临时 preview 文件、截图或本机配置。
