...
Agent files are edited through the `Adapter` trait (`core/src/adapter.rs`, `json_adapter.rs`, `toml_adapter.rs`): `read` / `upsert(one server)` / `remove(one server)`. **These operate per-server and preserve all content outside that server's MCP node, including comments and formatting** (`jsonc-parser` CST for JSON/JSONC; `toml_edit` for TOML). A past data-loss bug came from parsing the whole file into a lossy generic value and serializing it back. Never restore that pattern, never replace a malformed/non-object MCP section, and never return, log, source-cache, or upload the complete agent config. Scanning and previews expose only parsed MCP entries; user-selected local sources are sanitized to MCP-only cache files. The only complete local copy MUX may create is the timestamped pre-write backup (`core/src/applier.rs`).
