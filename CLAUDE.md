...
Agent files are edited through the `Adapter` trait (`core/src/adapter.rs`, `json_adapter.rs`, `toml_adapter.rs`): `read` / `upsert(one server)` / `remove(one server)`. **These operate per-server and preserve all content outside that server's MCP node, including comments and formatting** (`jsonc-parser` CST for JSON/JSONC; `toml_edit` for TOML). `core/src/codec.rs` owns each built-in Agent's wire schema and transport limits; do not collapse explicit-type, URL-inferred, or Agent-specific formats back into generic serde. `core/src/safe_write.rs` provides optimistic concurrency checks and atomic replacement; Cline additionally uses its shared settings lock. `core/src/applier.rs` must abort a target if its uniquely named backup fails. A past data-loss bug came from parsing the whole file into a lossy generic value and serializing it back. Never restore that pattern, never replace a malformed/non-object MCP section, and reject duplicate JSON keys anywhere in the target entry because their runtime meaning is ambiguous. Never return, log, source-cache, or upload the complete agent config. Scanning and previews expose only parsed MCP entries; user-selected local sources are sanitized to MCP-only cache files. The only complete Agent-file copy MUX may create is the timestamped pre-write backup; disable may persist only the target server's semantic entry, and must verify that exact value still matches before removal.
...
### Shared defaults
`data/agents.json` (21 agents) and `data/registry.json` are the single source of truth, embedded by `mux-core` via `include_str!` (`core/src/agents.rs`, `registry.rs`) — so both front-ends share them. Edit those JSON files directly. Built-ins without a verified global config path must use `global: null`; do not invent paths from undocumented app caches.
