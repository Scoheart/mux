...
All user data is one `~/.mux/settings.json`. **`mux-core` fully types only the sections it owns and passes the rest through opaquely** (via `#[serde(flatten)] extra`) so a future co-writer isn't clobbered. Every mutation is **strict read → modify one section → optimistic compare → atomic private write** (`mutate_settings` under a `static Mutex`; cross-process changes fail instead of being overwritten; Unix mode `0600`). Never mutate a stale whole-section snapshot when a single-entry settings mutation can run inside this lock.

### Agent config adapters — never rewrite the whole section
Agent files are edited through the `Adapter` trait (`core/src/adapter.rs`, `json_adapter.rs`, `toml_adapter.rs`, `toml_list_adapter.rs`, `yaml_adapter.rs`): `read` / `upsert(one server)` / `remove(one server)`. **These operate per-server and preserve all content outside that server's MCP node, including comments and formatting** (`jsonc-parser` CST for JSON/JSONC; `toml_edit` for TOML maps and array-of-tables; `yaml-edit` for YAML maps/lists). `core/src/codec.rs` owns each built-in Agent's wire schema and transport limits; do not collapse explicit-type, URL-inferred, list-layout, or Agent-specific formats back into generic serde. `core/src/safe_write.rs` provides optimistic concurrency checks and atomic replacement; Cline additionally uses its shared settings lock. `core/src/applier.rs` must abort a target if its uniquely named backup fails. A past data-loss bug came from parsing the whole file into a lossy generic value and serializing it back. Never restore that pattern, never replace a malformed/non-object MCP section, reject duplicate target keys, and reject YAML multi-document files because their runtime ownership is ambiguous. Never return, log, source-cache, or upload the complete agent config. Scanning and previews expose only parsed MCP entries; user-selected local sources are sanitized to MCP-only cache files. The only complete Agent-file copy MUX may create is the timestamped pre-write backup; on Unix its directory/file modes must remain `0700`/`0600`. Disable may persist only the target server's semantic entry, and must verify that exact value still matches before removal.

Built-in Agent field differences live in `core/src/codec.rs`; adapter selection must use `get_agent_adapter`, not the generic source adapter. A codec owns only connection fields and must preserve target-entry policy fields such as OAuth, tool lists, approval modes, timeouts, and enable flags. Defaults may be added only when absent on a new or existing entry, never overwrite an existing user value.

### Scope boundary
MUX's desktop App, TUI, and CLI manage **global Agent config only**. The desktop Tauri command contract resolves global paths internally and does not accept a project path/scope; keep it that way. Legacy `project` metadata and core helpers remain only for backward-compatible settings deserialization and isolated tests, not as a product feature.
...
### Shared defaults
`data/agents.json` is the deep-audited source of truth (39 definitions, 37 writable); `data/agent-catalog.json` is the broad discovery-only catalog (175 records, 191 unique after merge); `data/registry.json` is the curated MCP collection. All are embedded by `mux-core` via `include_str!` so both front-ends share them. Verified definitions override catalog aliases. Refresh the public catalog with `node scripts/update-agent-catalog.mjs`; promote a record into `agents.json` only after its global path, layout, codec, and transports are verified and round-trip tested. Catalog-only or otherwise unknown targets must keep `global: null`, `format: unknown`, and no transports so adapter selection fails closed. Do not invent paths from undocumented app caches. See `docs/agent-catalog.md`.
