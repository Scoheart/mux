// 端到端集成测试：headless 真实跑全局安装流程，验证配置文件被正确写入、覆写生效、备份生成。
...
fn install_flow_writes_config_applies_override_and_backs_up() {
    ...
    // —— 1) 预览：filesystem 装到 claude-code 的全局配置（~/.claude.json）——
    ...
    assert!(
        plan[0].file_path.ends_with(".claude.json"),
        "目标应为 .claude.json，实际 {}",
        plan[0].file_path
    );
    assert!(
        plan[0].config_json.contains("command"),
        "预览配置应含 command"
    );
    ...
    let mcp_file = home.join(".claude.json");
    let written = fs::read_to_string(&mcp_file).expect(".claude.json 应已写入");
    assert!(written.contains("mcpServers"), "应含 mcpServers 键");
    assert!(written.contains("filesystem"), "应含 filesystem 服务器");
    let v: serde_json::Value = serde_json::from_str(&written).unwrap();
    assert!(
        v["mcpServers"]["filesystem"]["command"].is_string(),
        "filesystem.command 应存在"
    );
    ...
    let backup_names: Vec<String> = fs::read_dir(&backups)
        .unwrap()
