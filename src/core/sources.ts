...
import { loadSettings, mutateSettings, writeAtomic, type Settings } from "./settings.js";
...
function cachedPath(def: SourceDef): string | null {
...
function readSection(format: string, key: string, path: string): Record<string, McpConfig> {
  const adapter = pickAdapter(format, key);
...
function parseFile(path: string, format: string, key: string, origin: RegistryOrigin): RegistryEntry[] {
...
function writeArray(path: string, list: RegistryEntry[]): void {
  writeAtomic(path, JSON.stringify(list, null, 2) + "\n");
}
