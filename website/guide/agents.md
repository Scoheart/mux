...
- **Qoder IDE / CLI / Desktop**：三个独立入口。IDE（原 `qoder`）继续使用 `~/.qoder/mcp.json`；新 Desktop（`qoder-desktop`，0.1.x）与 CLI（`qoder-cli`）共用 `~/.qoder/settings.json`，修改同名 MCP 会影响两者。Desktop 的 Models 通过 Settings → Models 配置，CLI 通过 `/model`；新版 Desktop Skills 的本地写入契约尚未核验。
