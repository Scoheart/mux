...
- **核验定义**：56 个逐项核验的 Agent 定义，其中 46 个有稳定的用户级全局 MCP 配置文件，可由 MUX 安全读写；另有 9 个 Skills-only 目标和只读的 Devin。
- **发现目录**：来自公开 MCP 客户端目录与官方客户端矩阵，只作为后续核验的数据储备。目录包含 **201 条**记录，与核验定义合并去重后共保留 **211 个** Agent identity，但不再作为单独标签页展示。
...
下表聚焦 MCP 契约：列出 46 个可写目标，并保留 Devin 作为明确的只读对照；Skills-only 定义见下方 Skills 能力与 [用户级 Skills](/guide/skills#已核验的-agent-路径)。
...
- **Qoder Desktop / Qoder CLI**：两者是独立 Agent。Qoder Desktop 的 MCP 页面编辑 `~/.qoder/mcp.json`；Qoder CLI 的 user scope 使用 `~/.qoder/settings.json`，MUX 分别扫描和写入，不会静默合并两份 MCP 列表。`SharedClientCache/mcp.json` 可能仍包含运行时镜像，但不是用户可编辑的权威配置。
