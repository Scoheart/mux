...
只有**已启用**来源参与目录拼装。来源模型支持启停，TUI 的来源屏幕可直接切换；桌面 `v1.1.4` 目前提供过滤、刷新和删除，暂未暴露启停开关。
...
Agent 的 JSON / TOML 配置采用“解析 → 增删目标条目 → 重新序列化”：其它顶层键、其它 server 和未建模字段会保留，但缩进与键顺序可能变化；写入前会先备份原文件。
