...
顶部 **Skills** 工作区先从公开 GitHub、本地文件夹或 Skill 压缩包解析候选，在本机审阅文件、风险和冲突后只写一份中央副本。然后从 Agent 页单独管理消费关系，把中央副本链接到已核验目录；共享一个物理 target 的 Agent 作为不可拆分组一起选择和审阅。Agent 页面不出现来源解析、安装或编辑流程。
