...
三类顶层资源共用同一套卡片结构：名称与状态在上，摘要和元数据居中，消费者在底部。卡片本身只负责选择资源，不再常驻复制、编辑或删除按钮；资产生命周期和“管理 Agent”集中在右侧详情面板。只有用户拥有的 MCP source copy 可编辑或删除；订阅与导入文件中的条目由其来源管理，仅在 Agent 文件中发现的外部配置不属于 Registry。
...
同一关系也可从中央资产详情的“管理 Agent”反向修改；两个入口调用同一个 planner。MUX 当前只管理 Agent 的用户级全局配置。
...
仅在 Agent 中扫描到的配置显示为只读“外部配置”，不会自动写进 Registry 或建立关系。中央资产删除则会先列出全部消费者，确认后原子清理所有关系、受管目标与中央记录。
...
悬停来源行后，远程订阅和本地文件可刷新；非受管来源可以删除。Agent 重新扫描只更新 observed inventory，不会刷新后自动生成 `discovered` 中央条目。删除来源会移除其缓存与目录条目，但已有消费关系需要通过影响计划处理。
...
顶部 **Models** 页面与 MCPs 使用同一套筛选栏、搜索、资源卡片和右侧详情面板。创建 Profile 只保存中央资产；之后可在详情面板或 Agent 页选择消费者，每个 Agent 同时最多一个兼容 Profile。编辑会传播到全部消费者，删除会级联清理关系和受管目标。API Key 只写入 macOS Keychain，不进入 `~/.mux/settings.json`、计划、Agent 配置预览或备份。
...
顶部 **Skills** 工作区先从公开 GitHub、本地文件夹或 Skill 压缩包解析候选，在本机审阅文件、风险和冲突后只写一份中央副本。然后通过 Skill 详情或 Agent 页单独管理消费关系，把中央副本链接到已核验目录；共享一个物理 target 的 Agent 作为不可拆分组一起选择和审阅。Agent 页面不出现来源解析、安装或编辑流程。
