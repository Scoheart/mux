# 桌面 App 指南

桌面 App 是 MUX 的可视化前端（macOS，Tauri + React），用于浏览目录、管理来源、安装 MCP 和编辑 Agent。它与命令行共享 `~/.mux/`。

> 还没安装？请先看 [安装](/guide/install#桌面-app-macos)。

## 界面总览

打开 App 默认进入 **Registry（目录）**。当前 `v1.1.4` 的主界面由以下区域组成：

![MUX v1.1.4 Registry 界面](/img/registry-overview-current.jpg)

| 区域 | 作用 |
|---|---|
| **Registry** | 从任意 Agent 页面返回目录。 |
| **Agent 图标条** | 进入某个 Agent，查看并管理它的全局 MCP；图标条可横向滚动。 |
| **虚线 `+`** | 新增自定义 Agent，位置紧邻 Agent 图标条。 |
| **主题 / 重新扫描 / 检查更新** | 切换外观、重读各 Agent 配置、手动检查正式版更新。 |
| **来源栏** | 按来源过滤目录；顶部提供“添加订阅”和“导入配置”。 |
| **目录工具栏** | 搜索、查看被覆盖项、粘贴配置、导出生效配置、新建 MCP。 |
| **MCP 卡片网格** | 展示名称、传输、来源、端点、使用情况及冲突状态。 |

## 目录与覆盖关系

Registry 默认显示所有已启用来源中的**每一份副本**。同一个 `name::transport` 出现在多个来源时，优先级较高的副本生效，其余副本继续显示，不会被隐藏：

- 生效副本保持普通卡片样式，不重复显示“生效”标签。
- 被覆盖副本使用靛蓝色强调、标记 **被覆盖**，并注明“以某来源为准”。
- 只有当前范围存在冲突时，搜索框右侧才出现 **被覆盖 N**；点击后只看被覆盖副本，再点一次恢复全部。
- 点击左侧某个来源，会查看该来源的全部副本；“被覆盖”筛选仍只作用于当前来源。

最终用于安装与导出的，是每个组合键优先级最高的那一份。完整规则见 [优先级](/guide/concepts#优先级-去重规则)。

## 卡片怎么看

每张卡片包含：

| 部件 | 含义 |
|---|---|
| 彩色头像与名称 | MCP 身份。 |
| `STDIO` / `HTTP` | 归一化后的传输类别；`sse` 和 `streamable-http` 归入 HTTP 身份。 |
| 来源 | Agent 名、手动添加、本地文件或订阅名称。 |
| 端点 | stdio 启动命令或 HTTP URL，过长时截断。 |
| 使用状态 | 绿点表示被若干 Agent 使用；灰点表示未使用。 |

悬停卡片后会显示**复制配置 / 编辑 / 删除**操作。只有“手动添加”和“自动探索”条目可以编辑或彻底删除；订阅与导入文件中的条目由其来源管理。点击卡片主体可打开详情。

## 安装到 Agent

1. 在顶部 Agent 图标条选择一个 Agent。
2. 点击 **添加 MCP**，搜索并选择尚未安装的条目。
3. MUX 先备份目标文件，再按该 Agent 的格式与配置键写入全局配置。

MUX 当前只管理 Agent 的全局安装。

## 开关与删除

Agent 页面中的每个已安装 MCP 都有开关和删除操作：

- **停用**：从 Agent 文件移除，但在 MUX 中保存完整配置，之后可以恢复。
- **启用**：把保存的配置重新写回 Agent 文件。
- **删除**：从该 Agent 卸载。

在 Registry 中删除“手动添加 / 自动探索”条目，会同时从目录和所有全局 Agent 中移除该条目，并保留备份。

## 编辑、粘贴与导出

- **编辑**：修改用户拥有的条目。保存后只自动传播到仍与旧目录配置一致的全局安装。
- **重新同步**：编辑页底部可显式重刷全局安装；默认跳过被手动定制的配置，确认后可强制覆盖。
- **粘贴配置**：支持 `mcpServers` JSON 或可识别的 TOML，解析后加入“手动添加”。
- **导出生效配置**：工具栏下载图标导出完整的去重后目录，不只导出手动条目；CLI 对应 `mux export`。

## 来源管理

来源栏顶部只有两个新增动作：

- **添加订阅**：输入远程配置 URL；弹窗内的 **Mux 精选**会填入官方精选订阅。
- **导入配置**：选择本地 JSON / TOML 配置文件。

悬停来源行后，远程订阅、本地文件和自动探索可刷新；非受管来源可以删除。删除来源会移除缓存与目录条目，但不卸载已经写入 Agent 的配置。

来源模型本身支持启停，TUI 的“来源”屏幕用 `Space` / `Enter` 操作。桌面 `v1.1.4` 的来源栏暂未提供启停开关。

## Agent 管理

- 顶部 Agent 图标条右侧的虚线 `+` 用于新增自定义 Agent。
- 进入 Agent 页面后，可以编辑全局路径、格式和配置键。
- 位于用户主目录内的路径保存为 `~/…`；主目录外的绝对路径保持原值。

内置的 21 个 Agent 见 [支持的 Agent](/guide/agents)。

## 自动更新与 CLI

- App 启动约 2.5 秒后静默检查最新**正式版**；失败不会打断使用。
- 顶部 **检查更新 vX.Y.Z** 可随时手动检查，并在失败时显示错误。
- 下载在后台进行；完成后可立即重启或下次启动时生效。
- 正式版 App 自带 CLI，并在启动后维护 `~/.local/bin/mux` 软链。详见 [安装](/guide/install#方式一随桌面-app-安装推荐)。

## 写入保证

- 每次修改 Agent 配置前，先把原文件复制到 `~/.mux/backups/`。
- 增删目标 server 时，保留其它顶层键、其它 server 和未建模字段。
- JSON / TOML 会重新序列化，缩进和键顺序等纯格式可能变化。
- `~/.mux/settings.json` 使用临时文件加重命名的原子写入。

命令行提供同一套核心能力 → [命令行 / TUI](/guide/cli)。
