...
MUX 桌面端把模型端点保存为中央 Model Profile，再由多个兼容 Agent 消费。创建 Profile 本身不会写 Agent 配置；每个 Agent 同时最多选择一个当前 Profile。当前可自动配置 Claude Code、Codex、Grok Build 和 Pi；Qoder 与 MiniMax Code 保留安全引导。
...
| Grok Build | 自动配置 | `~/.grok/config.toml` | Anthropic Messages / OpenAI Responses / OpenAI Chat Completions |
| MiniMax Code | 安全引导 | `~/.mavis/config.yaml`（MUX 自动管理独立的 `~/.mavis/mcp.json`） | Anthropic Messages / OpenAI Responses / OpenAI Chat Completions |
| Pi | 自动配置 | `~/.pi/agent/models.json` + `settings.json` | Anthropic Messages / OpenAI Responses / OpenAI Chat Completions |
| Qoder | 官方引导 | `~/.qoder/settings.json`（MUX 不写） | 在 Qoder 的 `/model` 中选择 |

Grok Build 原生支持 `chat_completions`、`responses` 与 `messages`。MUX 写入独立的 `[model.<mux-id>]` 并切换 `[models].default`，同时保留同文件中的 MCP、其他模型、权限和工具设置；需要认证时，Profile 只保存官方 `env_key` 变量名，密钥值仍由 Grok Build 的运行环境提供。Qoder 没有公开安全的非交互凭据写入接口；MiniMax Code 当前自定义 provider 会把 `options.apiKey` 作为字面量保存，因此两者仍由自身界面管理。
