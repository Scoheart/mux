...
MUX 桌面端可以保存一次模型端点，再应用到多个兼容 Agent。当前范围包括 Claude Code、Codex、Pi，以及仅提供引导的 Qoder、Grok Build 和 MiniMax Code，用于验证配置语义与安全边界。
...
模型卡片只用于选择，不常驻编辑或删除按钮；管理操作集中在右侧详情面板。新建与编辑使用统一编辑对话框，应用和删除会先进入审阅对话框；写入进行中时不能意外关闭，失败后可在原位置检查原因并重试。
