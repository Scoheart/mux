...
MUX Desktop 可以统一查看、安装和分配符合 Agent Skills 格式的用户级 Skills。当前版本只管理用户主目录中的全局 Skill，不读取或写入项目目录中的 `.agents/skills`、`.claude/skills` 等内容。
...
私有 GitHub 仓库、GitLab、SSH Git 和任意压缩包 URL 当前不受支持。

## 一份中央副本，多处链接

确认安装后，MUX 把每个 Skill 的唯一托管副本放在：

```text
~/.mux/skills/<skill-name>/
```

选中的 Agent 目录中只创建指向中央副本的受管链接。这样一次更新会被所有已分配 Agent 看到，而停用某个分配只移除对应链接，不会删除中央内容。

MUX 按物理目录归一化分配。某些 Agent 会兼容读取另一个目录，例如 Cursor、Gemini CLI、OpenCode 和 GitHub Copilot CLI 都可以读取 `~/.agents/skills`。因此向 Codex 的首选目录写入链接时，其他已安装 Agent 也可能同时获得访问。审阅页会列出实际受影响的全部 Agent，并去掉会导致同一 Skill 被重复发现的冗余链接。
...
| 操作 | 结果 |
|---|---|
| 检查 / 更新 | 后台和手动检查只读取 GitHub revision 或本地哈希，不改变正文。选择更新后才下载候选、展示差异、重新审计并确认替换；中央副本的本地修改会要求“备份后替换”。 |
